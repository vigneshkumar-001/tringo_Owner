...
import '../Model/offer_products.dart';
...
class _OfferProductsState extends ConsumerState<OfferProducts> {
  // selection should be safe even if list changes / filtered
  ...
  Widget build(BuildContext context) {
    ...
    final filtered = _filteredItems(offerProductsData?.data);
                ...
                Padding(
                  ...
                  child: Column(
                    children: [
                      ...
                      _buildCategoryChips(offerProductsData?.data),
                      SizedBox(height: 20),
                        ...
                        itemBuilder: (context, index) {
                          ...
                          return GestureDetector(
                            onTap: () {
                              setState(() {
                                if (isSelected) {
                                  _selectedProductIds.remove(data.id);
                                } else {
                                  _selectedProductIds.add(data.id);
                                }
                              });
                            },
                            child: _buildProductTile(
                              ...
                              url: data.imageUrl ?? '',
                              offerPrice: (data.offerPrice ?? 0).toInt(),
  ...
  //  Filter items by selected categoryId
  List<ProductItem> _filteredItems(OfferProductsData? data) {
    final items = data?.items ?? [];
    if (_selectedCategoryId == null || _selectedCategoryId!.isEmpty) {
      return items;
    }
    return items.where((e) => e.category == _selectedCategoryId).toList();
  }

  //  Build category chips from API categories
  Widget _buildCategoryChips(OfferProductsData? data) {
    final categories = data?.categories ?? [];
    final labels = <String>[...categories.map((e) => e.label)];
...
// import 'package:tringo_owner/Core/Widgets/bottom_navigation_bar.dart';
// import 'package:tringo_owner/Presentation/Create%20App%20Offer/Controller/offer_notifier.dart';
// import 'package:tringo_owner/Presentation/No%20Data%20Screen/Screen/no_data_screen.dart';
// import '../../../Core/Const/app_color.dart';
