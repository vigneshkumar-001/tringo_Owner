...
class TringoOverlayService : Service() {
    ...
    private fun showOverlay(phone: String, contactName: String, preferCache: Boolean) {
        ...
        serviceScope.launch {
            try {
                ...
                val cardAny = map(dataAny["card"])

                val cardTitle = str(cardAny["title"]).trim()
                val cardSubtitle = str(cardAny["subtitle"]).trim()
                val cardImageUrl = str(cardAny["imageUrl"]).trim()

                val detailsAny = map(cardAny["details"])
                val cat = str(detailsAny["category"]).trim()
                val opensAt = str(detailsAny["opensAt"]).trim()
                val closesAt = str(detailsAny["closesAt"]).trim()
                val addr = str(detailsAny["address"]).trim()
                ...
                val cards = listAny.mapIndexed { idx, item ->
                    val m = map(item)
                    OverlayAdCard(
                        id = str(m["id"]).ifBlank { "ad_$idx" },
                        title = str(m["englishName"]).ifBlank { str(m["title"]).ifBlank { "Ad ${idx + 1}" } },
                        subtitle = buildAdSubtitleMap(m),
                        rating = str(m["rating"]).toDoubleOrNull(),
                        ratingCount = str(m["ratingCount"]).toIntOrNull(),
                        openText = str(m["openLabel"]).ifBlank { null },
                        isTrusted = bool(m["isTrusted"]),
                        imageUrl = str(m["primaryImageUrl"]).ifBlank { str(m["imageUrl"]) }
                    )
                }
    ...
    private fun buildAdSubtitleMap(m: Map<String, Any?>): String {
        val addr = str(m["addressEn"]).ifBlank { str(m["addressTa"]) }
        val city = str(m["city"])
        val state = str(m["state"])
        val country = str(m["country"])
    ...
    private fun map(v: Any?): Map<String, Any?> {
        return (v as? Map<*, *>)?.entries?.associate { (k, value) -> k.toString() to value } ?: emptyMap()
    }

    private fun list(v: Any?): List<Any?> {
        return (v as? List<*>)?.toList() ?: emptyList()
    }
    ...
    private fun bool(v: Any?): Boolean {
        return when (v) {
            is Boolean -> v
            is Number -> v.toInt() != 0
            is String -> v.trim().lowercase() in listOf("true", "1", "yes")
            else -> false
        }
    }
}
